...
| 12 — Release readiness y handoff | Hecha | Cerrada con el release **`v1.0.0`** (commit `53d79a4`), publicado en GHCR como `reservahub-app` y `reservahub-web` — imágenes públicas, digests en `docs/RELEASE.md` §2. `docs/DEPLOYMENT_HANDOFF.md` reescrito para el modelo VPS multiproyecto, `docs/RELEASE.md`, `README.md` propio, `.github/workflows/{ci,release}.yml`, `docker/production/{app,web}.Dockerfile`, `compose.production.yaml`, `php artisan demo:reset` y `demo:restore-access` con las guardas de `DemoEnvironment`, `trustProxies` configurado. Repositorio público en `github.com/Gonzalez-Luciano/ReservaHub`. El deployment real al VPS queda fuera del alcance de este repositorio (ver `docs/DEPLOYMENT_HANDOFF.md`) |
