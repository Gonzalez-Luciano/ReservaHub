...
class WebhookConcurrencyTest extends TestCase
{
    ...
    public function test_two_sessions_cannot_both_claim_the_same_event_identity(): void
    {
        ...
        // A commitea antes de que B intente: un solo hilo PHP no puede tener dos
        // llamadas bloqueantes en vuelo a la vez, y un INSERT...ON CONFLICT DO
        // NOTHING sin resolver SÍ bloquea contra una fila en conflicto todavía no
        // comprometida (a diferencia de FOR UPDATE SKIP LOCKED, que no bloquea).
        // Dejar a A sin commit antes del intento de B deadlockearía el proceso.
